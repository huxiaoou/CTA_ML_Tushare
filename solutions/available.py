...
def load_major(db_struct_instru: CDbStruct, bgn_date: str, stp_date: str) -> pd.DataFrame:
    ...
    amt_data = sqldb.read_by_range(bgn_date, stp_date, value_columns=["trade_date", "amount"])
    return amt_data
...
def get_available_universe_by_date(x: pd.Series, ret_df: pd.DataFrame, amt_df: pd.DataFrame) -> pd.DataFrame:
    ...
    sub_available_universe_df["trading_day"] = trade_date
...
def get_available_universe(
    ...
    for instru in cfg_avlb_unvrs.universe:
        ...
        selected_major_data = instru_major_data.set_index("trade_date")
        ...
        return_data[instru] = selected_major_data["return_c_major"]
    ...
    update_df.sort_values(by=["trade_date", "amount"], ascending=[True, True, False], inplace=True)
    update_df = update_df[["trade_date", "return", "amount"]]

    # --- add section
    unvrs = cfg_avlb_unvrs.universe
    update_df["sectorL0"] = update_df["instrument"].map(lambda z: unvrs[z].sectorL0)
    update_df["sectorL1"] = update_df["instrument"].map(lambda z: unvrs[z].sectorL1)
