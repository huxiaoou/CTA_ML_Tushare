...
from solutions.shared import gen_nav_db, group_sim_args
from typedef import CSimArgs
from typedef import TSimArgsPriKey, TSimArgsPriVal, TPid
...
class CEvlMdl(CEvl):
    ...
    def add_arguments(self, res: dict):
        ret_class, trn_win, model_desc, unique_id, ret_name, tgt_ret_name = self.sim_args.sim_id.split(".")
        other_arguments = {
            "ret_class": ret_class,
            "trn_win": trn_win,
...
def main_eval_mdl(
    ...
    # --- print best models for each ret name
    for ret_name, ret_name_data in evl_data.groupby(by="ret_name"):
        print("-" * 180)
        print(f"{SFG(ret_name)} models with Best sharpe+calmar")  # type:ignore
        print(ret_name_data.head(5))

    # --- print sector models picked for portfolio
    print("-" * 180)
    print(f"{SFG('models picked for portfolio')}")
    for ret_name, ret_name_data in evl_data.groupby(by="ret_name"):
...
def process_for_plot(
        ...
        sub_grouped_sim_args: TSimArgsPriVal,
        simu_mdl_dir: str,
        fig_save_dir: str,
        bgn_date: str,
        stp_date: str,
    ...
    for unique_id, sim_args in sub_grouped_sim_args.items():
        s = CEvlMdl(sim_args, simu_mdl_dir)
        ret_data[unique_id] = s.get_ret(bgn_date, stp_date)
